...
void grupo_cercano (int nvec, float mvec[][NDIM], float cent[][NDIM],int *popul){
	...
	for(int j=0;j<nvec; j++){
		distmenor = FLT_MAX;//gendist(mvec[j],cent[0]);
		imenor = 0;
   ...
   float b[ngrupos];
    ...
    for(int i =0; i< ngrupos ;i++){
	 ...
	 for(int j = 0; j < listag[i].nvecg -1 ; j++){
		float dist = (float) gendist(mvec[listag[i].vecg[j]],mvec[listag[i].vecg[j+1]] );
		sum = sum + dist;
		//sum = sum + listag[i].vecg[j];
	}

	sum = sum /(float)listag[i].nvecg;
	if(listag[i].nvecg < 2){
		sum = 0.0;
		a[i]= 0.0;
	}
...
//funcion ordena un array de menor a mayor
...
void analisis_campos(struct lista_grupos *listag, float mcam[][NCAM],
	...
	for(int i=0;i<NCAM;i++){
	   ...
	   for(int j=0;j<ngrupos;j++){
		...
		//la mediana de las probabilidades del grupo j se almacena en el array mediana.
