...
void grupo_cercano (int nvec, float mvec[][NDIM], float cent[][NDIM],int *popul){
	...
	int imenor;
	int i;
	#pragma omp parallel for schedule(runtime) private(i,distmenor,distancia,imenor)
...
{   int k,j;
    float sum,dist;
    #pragma omp for schedule(runtime) private(sum,dist,k,j)
    for(int i =0; i< ngrupos ;i++){
	 float sum = 0.0;
    ...
    float sumc,distc;
    #pragma omp for schedule(runtime) private(sumc,distc)
...
void analisis_campos(struct lista_grupos *listag, float mcam[][NCAM],
	...
	#pragma omp parallel for schedule(runtime) private(j,m)
